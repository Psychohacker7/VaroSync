          ...
          <div className="px-5 pt-8 pb-8 md:px-12 md:pt-11 md:pb-10">
            ...
            <div className="relative mx-2 mb-2" style={{ height: 86 }}>
              ...
              {PROGS.map((prog, i) => {
                ...
                return (
                  <button key={prog.name} type="button" onClick={() => setSel(i)} className="absolute group" style={{ left: `${prog.pos * 100}%`, top: 53, transform: "translate(-50%,-50%)" }}>
                    {/* label sits above the dot; on phones only the active marker shows (avoids collision on the narrow track) */}
                    <span className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap text-[11.5px] transition-colors ${on ? "block text-[#0a0a0a] font-bold" : "hidden md:block text-[#888377] group-hover:text-[#0a0a0a]"}`}>{prog.name}</span>
                    <span className="block rounded-full transition-all" style={{ width: on ? 16 : 11, height: on ? 16 : 11, background: c, boxShadow: on ? `0 0 0 5px ${c}22` : "none" }} />
                  </button>
                );
              })}
            </div>
            ...
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={sel} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.28, ease } }} exit={{ opacity: 0, transition: { duration: 0.1 } }}
                className="mt-8 pt-7 border-t border-[#efefed] grid md:grid-cols-[260px_1fr] gap-6 md:gap-8">
                <div>
                ...
                <div className="border-t border-[#efefed] pt-6 md:border-t-0 md:pt-0 md:border-l md:border-[#efefed] md:pl-8 flex items-center">
